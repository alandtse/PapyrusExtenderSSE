...
namespace Papyrus::ObjectReference
{
		...
		if (const auto processLists = RE::ProcessLists::GetSingleton(); processLists) {
			...
			if (!result.empty()) {
				for (auto& enemy : result) {
					if (enemy && enemy->HasMagicEffect(a_effect)) {
						hasEffect = true;
						break;
					}
				}
	...
	void SetLinkedRef(STATIC_ARGS, RE::TESObjectREFR* a_ref, RE::TESObjectREFR* a_targetRef, RE::BGSKeyword* a_keyword)
	{
		if (!a_ref) {
			a_vm->TraceStack("Object reference is None", a_stackID);
			return;
	...
	void Bind(VM& a_vm)
	{
		...
		BIND(GetMotionType);
		BIND(GetNumActorsWithEffectInRange);
