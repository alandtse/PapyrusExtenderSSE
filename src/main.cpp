...
SKSE_PLUGIN_LOAD(const SKSE::LoadInterface* a_skse)
{
	SKSE::Init(a_skse, { .log = true,
						   ...
						   .trampoline = true,
#ifdef SKYRIMVR
						   // CommonLibVR's SKSE::Trampoline needs more per-hook headroom than
						   // upstream's 264 (sized for REL::Trampoline on SE/AE) - 264 wasn't
						   // enough for all ~18 hooks and crashed with "Failed to handle
						   // allocation request" during Install() (see extern/CommonLibVR/src/SKSE/Trampoline.cpp).
						   .trampolineSize = 1024
#else
						   .trampolineSize = 264
#endif
